...
/*   Updated: 2025/09/18 14:08:18 by tignatov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	find_map_dim(t_map *map, char **file, size_t i)
{
	...
	k = i;
	map->start_index = find_start_index(file[i]);
	while (file[i] && ft_strchr(file[i], '1'))
	{
		new_min_index = find_start_index(file[i]);
		if (new_min_index < map->start_index)
			map->start_index = new_min_index;
		i++;
	}
	i = k;
	while (file[i] && ft_strchr(file[i], '1'))
	{
		...
		if (new_max_index > map->end_index)
			map->end_index = new_max_index;
		i++;
	}
	map->max_len = map->end_index - map->start_index + 1;
}
